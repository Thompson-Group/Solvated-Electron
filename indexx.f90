      subroutine indexx(comparx)

      use kinds
      implicit none

      integer(kind=ip), parameter :: M=7, NSTACK=50
      integer(kind=ip), dimension(NSTACK) :: istack
      integer(kind=ip) :: i, indxi, indxt, ir, itemp, j, jstack, k, l
      logical, external :: compar

      do j = 1, ng
        indx(j) = j
      enddo

      jstack = 0
      l = 1
      ir = ng
 1    if(ir-l.lt.M)then
         do 13 j=l+1,ir
            indxt=indx(j)
            do 12 i=j-1,l,-1
               indxi = indx(i)
               if(compar(indxt,indxi)) goto 2
               indx(i+1)=indxi
 12         continue
            i=l-1
 2          indx(i+1)=indxt
 13      continue
         if(jstack.eq.0) goto 40
         ir=istack(jstack)
         l=istack(jstack-1)
         jstack=jstack-2
      else
         k=(l+ir)/2
         itemp=indx(k)
         indx(k)=indx(l+1)
         indx(l+1)=itemp
         if(compar(indx(l),indx(ir))) then
            itemp=indx(l)
            indx(l)=indx(ir)
            indx(ir)=itemp
         endif
         if(compar(indx(l+1),indx(ir))) then
            itemp=indx(l+1)
            indx(l+1)=indx(ir)
            indx(ir)=itemp
         endif
         if(compar(indx(l),indx(l+1))) then
            itemp=indx(l)
            indx(l)=indx(l+1)
            indx(l+1)=itemp
         endif
         i=l+1
         j=ir
         indxt=indx(l+1)
 3       continue
         i=i+1
         if(compar(indxt,indx(i))) goto 3
 4       continue
         j=j-1
         if(compar(indx(j),indxt)) goto 4
         if(j.lt.i) goto 5
         itemp=indx(i)
         indx(i)=indx(j)
         indx(j)=itemp
         goto 3
 5       indx(l+1)=indx(j)
         indx(j)=indxt
         jstack=jstack+2
         if(jstack.gt.NSTACK)pause 'NSTACK too small in indexx'
         if(ir-i+1.ge.j-l)then
            istack(jstack)=ir
            istack(jstack-1)=i
            ir=j-1
         else
            istack(jstack)=j-1
            istack(jstack-1)=l
            l=i
         endif
      endif
      goto 1

 40   continue

      return
      end subroutine indexx



